...
class CornersProblem(search.SearchProblem):
    ...
    def is_goal_state(self, state):
        ...
        # goal state is when all corners are visited
    ...
    def get_successors(self, state):
        ...
        for action in [Directions.NORTH, Directions.EAST, Directions.SOUTH, Directions.WEST]:
            ...
            # if this next position is not a wall
            if not self.walls[next_position[0]][next_position[1]]:
                next_visited_corners = set(visited_corners)
                # if this next position is a corner spot 
                if next_position in self.corners:
                    next_visited_corners.add(next_position)
                # Create the new state with updated visited corners
                new_state = (next_position, tuple(next_visited_corners))
                successors.append((new_state, action, 1))  # Cost is always 1
...
class AStarFoodSearchAgent(SearchAgent):
    ...
    def __init__(self):
        self.search_function = lambda prob: search.a_star_search(prob, self.food_heuristic)
        self.search_type = FoodSearchProblem


    def food_heuristic(self, state, problem):
        """
        A heuristic for the FoodSearchProblem.

        Estimates the cost to eat all food based on the maze distance to the farthest food pellet.
        """
        position, foodGrid = state
        food_list = foodGrid.as_list()  # Convert food grid to a list of food positions

        if not food_list:
            return 0  # No food left → heuristic is 0

        # Compute the maze distance to the farthest food pellet
    # Compute the maximum maze distance to any unvisited food pellet
        max_distance = 0
        ...
        return max_distance  # The minimum cost must be at least the farthest food
...
class ClosestDotSearchAgent(SearchAgent):
    ...
    def find_path_to_closest_dot(self, game_state):
        ...
        # create a search problem where the goal is ANY food dot
        problem = AnyFoodSearchProblem(game_state)

        # solve the problem using BFS (since it's optimal for unweighted grids)
